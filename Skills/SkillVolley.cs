...
[CombatOverride("skill_Volley")]
public class SkillVolley : SkillBase
{
    public override string OverrideKey => "skill_Volley";
    ...
    public override IEnumerator Execute(CombatScript combat, int casterTeam, int casterSlot, ActionData action)
    {
        ...
        foreach (var t in targets)
        {
            ...
            if (dmg.baseRoll.HasValue) rollsToShow.Add(("Volley→" + t.unit.name, dmg.baseRoll.Value));
            if (dmg.critBonusRoll.HasValue) rollsToShow.Add(("Volley CRIT→" + t.unit.name, dmg.critBonusRoll.Value));
            if (dmg.amount > 0)
